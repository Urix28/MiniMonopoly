...
function asteroidInit(){
	var rando = Math.floor((Math.random() * 15) + 8);
	asteroidPool = Pool.create(Asteroid, rando);
	asteroids = [];
}
...
function renderScanlines(){
	...
	for(i; i > -1; --i){
		...
		context.strokeStyle = (Math.random() > 0.0001) ? '#e44c65' : '#222';
		context.stroke();
	}
